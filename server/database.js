...
if (questionCount.count === 0) {
  const insertQ = db.prepare('INSERT INTO quiz_questions (question, option_a, option_b, option_c, option_d, correct_answer, category, explanation, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)');
  seedQuizQuestions.forEach(q => insertQ.run(q.question, q.option_a, q.option_b, q.option_c, q.option_d, q.correct_answer, q.category, q.explanation));
  console.log(`Seeded ${seedQuizQuestions.length} quiz questions`);
}
