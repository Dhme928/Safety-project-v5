...
function loadProfileFromStorage() {
  try {
    const raw = localStorage.getItem('obsAppProfile');
    if (!raw) return;
    const parsed = JSON.parse(raw);
    if (parsed && typeof parsed === 'object') {
      userProfile.name = parsed.name || '';
      userProfile.id = parsed.id || '';
    }
  } catch (err) {
    console.warn('Could not load profile from storage', err);
  }
}

function saveProfileToStorage() {
  try {
    localStorage.setItem('obsAppProfile', JSON.stringify(userProfile));
  } catch (err) {
    console.warn('Could not save profile to storage', err);
  }
}

function applyProfileToObservationForm() {
  const nameInput = $('#addObsReporterName');
  const idInput = $('#addObsReporterId');
  if (!nameInput || !idInput) return;
  if (userProfile.name) nameInput.value = userProfile.name;
  if (userProfile.id) idInput.value = userProfile.id;
}

function applyProfileToPermitForm() {
  const nameInput = $('#addPermitReceiver');
  if (!nameInput) return;
  if (userProfile.name) nameInput.value = userProfile.name;
}
...
function setupProfileControls() {
  ...
  const loginBtn = $('#openLoginProfileButton');
  const saveBtn = $('#loginSaveButton');
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => {
      ...
      userProfile.id = idInput.value.trim();
      saveProfileToStorage();
      applyProfileToObservationForm();
      applyProfileToPermitForm();
      hideLoginModal();
    });
  }
}

async function fetchEvidenceList(category, refId) {
  const url = `/api/evidence?category=${encodeURIComponent(category)}&refId=${encodeURIComponent(refId)}`;
  const res = await fetch(url, { credentials: 'include' });
  if (!res.ok) throw new Error('Failed to fetch evidence');
  return res.json();
}

async function uploadEvidenceFile(category, refId, file) {
...
function renderEvidenceList(container, files) {
  ...
  if (!files || !files.length) {
    container.innerHTML = '<p class="text-muted">No uploaded photos yet.</p>';
    return;
  }

  container.innerHTML = '';
  const grid = document.createElement('div');
  grid.className = 'evidence-list-grid';
  ...
  files.forEach((file, index) => {
    ...
    if (isImage) {
      ...
      img.src = file.url;
      img.alt = file.originalName || `Evidence photo ${index + 1}`;
    ...
    link.textContent = file.originalName || 'Open';
    ...
    if (file.uploadedAt) {
      ...
      meta.textContent = new Date(file.uploadedAt).toLocaleString();
...
const POINTS_STORAGE_KEY = 'obsAppPointsV1';
...
function loadPointsFromStorage() {
  try {
    ...
    if (!raw) {
      pointsState.todayDate = getTodayKey();
      return;
    }
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') {
    ...
    Object.assign(pointsState, parsed);
    const today = getTodayKey();
    if (pointsState.todayDate !== today) {
      pointsState.todayDate = today;
      pointsState.todayObservationCount = 0;
      pointsState.todayPermitCount = 0;
      pointsState.todayHeavyCount = 0;
    }
  } catch (err) {
    console.warn('Could not load points from storage', err);
    pointsState.todayDate = getTodayKey();
  }
}
...
function savePointsToStorage() {
  try {
    localStorage.setItem(POINTS_STORAGE_KEY, JSON.stringify(pointsState));
  } catch (err) {
    console.warn('Could not save points to storage', err);
  }
}

function getLevelFromPoints(points) {
  if (points >= 500) return 'Level 5 – Safety Champion';
  if (points >= 300) return 'Level 4 – Role Model';
  if (points >= 150) return 'Level 3 – Safety Leader';
  if (points >= 60) return 'Level 2 – Safety Advocate';
  return 'Level 1 – Safety Beginner';
}

function updatePointsUI() {
  const totalEl = $('#myPointsTotal');
  const levelEl = $('#myPointsLevel');
  const challengeTextEl = $('#myPointsChallengeText');
  const challengeProgressEl = $('#myPointsChallengeProgress');

  if (totalEl) totalEl.textContent = String(pointsState.total || 0);
  if (levelEl) levelEl.textContent = getLevelFromPoints(pointsState.total || 0);

  // Simple daily challenge: add 3 observations today
  const target = 3;
  const progress = pointsState.todayObservationCount || 0;
  if (challengeTextEl) {
    challengeTextEl.textContent = 'Add 3 safety observations today.';
  }
  if (challengeProgressEl) {
    challengeProgressEl.textContent = `${progress} / ${target}`;
  }
}

function awardPoints(category, amount) {
  if (!amount || amount <= 0) return;
  pointsState.total = (pointsState.total || 0) + amount;
  ...
  if (category === 'observation') {
    pointsState.todayObservationCount = (pointsState.todayObservationCount || 0) + 1;
  } else if (category === 'permit') {
    pointsState.todayPermitCount = (pointsState.todayPermitCount || 0) + 1;
  } else if (category === 'heavy') {
    pointsState.todayHeavyCount = (pointsState.todayHeavyCount || 0) + 1;
  }
...
function showAddObservationModal() {
  const modal = $('#addObservationModal');
  if (modal) modal.classList.add('show');
  applyProfileToObservationForm();
}

function hideAddObservationModal() {
  const modal = $('#addObservationModal');
  if (modal) modal.classList.remove('show');
}

function showAddPermitModal() {
  const modal = $('#addPermitModal');
  if (modal) modal.classList.add('show');
  applyProfileToPermitForm();
}

function hideAddPermitModal() {
  const modal = $('#addPermitModal');
  if (modal) modal.classList.remove('show');
}

function showAddHeavyModal() {
  const modal = $('#addHeavyModal');
  if (modal) modal.classList.add('show');
}

function hideAddHeavyModal() {
  const modal = $('#addHeavyModal');
  if (modal) modal.classList.remove('show');
}
...
window.showAddObservationModal = showAddObservationModal;
window.hideAddObservationModal = hideAddObservationModal;
window.showAddPermitModal = showAddPermitModal;
window.hideAddPermitModal = hideAddPermitModal;
window.showAddHeavyModal = showAddHeavyModal;
window.hideAddHeavyModal = hideAddHeavyModal;
...
function showPermitDetail(permit) {
  ...
  // Load in-app evidence thumbnails if this observation came from the database
  try {
    if (obs && typeof obs.dbId === 'number') {
      const container = document.getElementById('obsDetailEvidenceList');
      if (container) {
        fetchEvidenceList('observation', obs.dbId)
          .then(files => {
            renderEvidenceList(container, files);
          })
          .catch(err => {
            console.error('Failed to load in-app observation evidence', err);
          });
      }
    } else {
      // If csv-based only, show a simple info message
      const container = document.getElementById('obsDetailEvidenceList');
      if (container) {
        container.innerHTML = '<p class="text-muted">No in-app photos for this item.</p>';
      }
    }
  } catch (err) {
    console.error('Error setting up observation evidence thumbnails', err);
  }
...
// -------------------- In-app add forms (Observations / Permits / Heavy) --------------------


async function submitNewObservation() {
  const reporterName = ($('#addObsReporterName') || {}).value?.trim() || '';
  const reporterId = ($('#addObsReporterId') || {}).value?.trim() || '';
  const area = ($('#addObsArea') || {}).value?.trim() || '';
  const riskLevel = ($('#addObsRiskLevel') || {}).value || '';
  const directCause = ($('#addObsDirectCause') || {}).value?.trim() || '';
  const description = ($('#addObsDescription') || {}).value?.trim() || '';
  const correctiveAction = ($('#addObsCorrectiveAction') || {}).value?.trim() || '';

  if (!reporterName || !area || !riskLevel || !description) {
    alert('Please fill Reporter Name, Area, Risk Level and Description.');
    return;
  }
  ...
  const saveBtn = $('#addObsSaveButton');
  if (saveBtn) {
  ...
  try {
    ...
    const evidenceInput = $('#addObsEvidenceInput');
    let hasEvidence = false;
    if (evidenceInput && evidenceInput.files && evidenceInput.files.length) {
      hasEvidence = true;
      for (const file of evidenceInput.files) {
        await uploadEvidenceFile('observation', created.id, file);
      }
    }

    // Push a lightweight version into the in-memory list so the user can see it right away
    try {
      if (Array.isArray(state.observations)) {
        const now = new Date();
        const newItem = {
          code: created.code || `OBS-${created.id}`,
          date: now,
          dateRaw: now.toISOString().slice(0, 10),
          day: now.toLocaleDateString(undefined, { weekday: 'short' }),
          group: reporterName || reporterId || 'Me',
          type: 'Observation',
          obsClass: '',
          obsTypes: '',
          injuryFlag: '',
        ...
        state.observations.unshift(newItem);
        renderObservationsList();
      }
    } catch (err) {
      console.warn('Could not append new observation to list', err);
    }

    awardPoints('observation', 20);

    alert('Observation saved successfully.');
    hideAddObservationModal();
  } catch (err) {
...
async function submitNewPermit() {
  const permitType = ($('#addPermitType') || {}).value || '';
  const area = ($('#addPermitArea') || {}).value?.trim() || '';
  const receiverName = ($('#addPermitReceiver') || {}).value?.trim() || '';
  const description = ($('#addPermitDescription') || {}).value?.trim() || '';

  if (!permitType || !area || !receiverName || !description) {
    alert('Please fill all required permit fields.');
    return;
  }

  const payload = { permitType, area, receiverName, description };

  const saveBtn = $('#addPermitSaveButton');
  if (saveBtn) {
    saveBtn.disabled = true;
    saveBtn.textContent = 'Saving...';
  }
  ...
  try {
    ...
    const evidenceInput = $('#addPermitEvidenceInput');
    if (evidenceInput && evidenceInput.files && evidenceInput.files.length) {
      for (const file of evidenceInput.files) {
        await uploadEvidenceFile('permit', created.id, file);
      }
    }

    awardPoints('permit', 15);
...
async function submitNewHeavyLog() {
  const equipmentName = ($('#addHeavyEquipmentName') || {}).value?.trim() || '';
  const area = ($('#addHeavyArea') || {}).value?.trim() || '';
  const status = ($('#addHeavyStatus') || {}).value || '';
  const description = ($('#addHeavyDescription') || {}).value?.trim() || '';

  if (!equipmentName || !area || !status) {
    alert('Please fill Equipment, Area and Status.');
    return;
  }

  const payload = { equipmentName, area, status, description };

  const saveBtn = $('#addHeavySaveButton');
  if (saveBtn) {
    saveBtn.disabled = true;
    saveBtn.textContent = 'Saving...';
  }
  ...
  try {
    ...
    const evidenceInput = $('#addHeavyEvidenceInput');
    if (evidenceInput && evidenceInput.files && evidenceInput.files.length) {
      for (const file of evidenceInput.files) {
        await uploadEvidenceFile('heavy', created.id, file);
      }
    }

    awardPoints('heavy', 10);

    alert('Heavy equipment log saved successfully.');
    hideAddHeavyModal();
  } catch (err) {
    console.error('Failed to save heavy log', err);
    alert('Failed to save heavy equipment log. Please try again.');
  } finally {
    if (saveBtn) {
      saveBtn.disabled = false;
      saveBtn.textContent = 'Save';
...
function setupAddFormsSaveButtons() {
  const obsSave = $('#addObsSaveButton');
  if (obsSave) {
  ...
  const permitSave = $('#addPermitSaveButton');
  if (permitSave) {
  ...
  const heavySave = $('#addHeavySaveButton');
  if (heavySave) {
    heavySave.addEventListener('click', () => {
      submitNewHeavyLog();
    });
  }
}
...
function showObservationDetail(obs) {
  ...
  body.innerHTML = `
    ...
    <section class="obs-detail-section">
      <div class="obs-detail-section-title">In-app uploaded photos</div>
      <div id="obsDetailEvidenceList" class="evidence-list"></div>
    </section>
...
function setupAddPermitButton() {
  const btn = $('#addPermitButton');
  if (!btn) return;
  btn.addEventListener('click', () => {
    showAddPermitModal();
  });
...
function setupAddHeavyButton() {
  const btn = $('#addHeavyEquipmentButton');
  if (!btn) return;
  btn.addEventListener('click', () => {
    showAddHeavyModal();
  });
...
function initApp() {
  ...
  setupAddPermitButton();
  setupAddHeavyButton();
  setupAddFormsSaveButtons();
